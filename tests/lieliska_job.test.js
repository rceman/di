...
describe("runLieliskaJob", () => {
  ...
  it("prefers exact date before exact amount when last 4 digits collide", () => {
    const preview = createPreview([
      ["A", "EARLIER", "11.02.2026.", "D", "X", "50", "EUR", "G", "R1", "50", "Given Shop", "981315370690143485", "", ""],
      ["A", "LATER", "15.02.2026.", "D", "X", "50", "EUR", "G", "R2", "50", "Goldlight Shop", "981986503392293485", "", ""],
    ]);
    preview.sourceEntries = [
      {
        svitrkods: "000000000000003485",
        summa: "50",
        datums: "2026-02-11 13:09",
        tirdzniecibasVieta: "Given by Grenardi",
      },
    ];

    const result = runLieliskaJob(preview);

    expect(result.rows[0][12]).toBe("000000000000003485");
    expect(result.rows[0][13]).toBe("50");
    expect(result.rows[1][12]).toBe("");
    expect(result.rows[1][13]).toBe("");
  });

  it("uses venue first-word as a tie-breaker after date and amount", () => {
    const preview = createPreview([
      ["A", "GIVEN-1", "11.02.2026.", "D", "X", "50", "EUR", "G", "R1", "50", "Given Akropole", "981315370690143485", "", ""],
      ["A", "GOLD-1", "11.02.2026.", "D", "X", "50", "EUR", "G", "R2", "50", "Goldlight Akropole", "981986503392293485", "", ""],
    ]);
    preview.sourceEntries = [
      {
        svitrkods: "000000000000003485",
